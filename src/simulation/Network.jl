...
function random_λ₊λ₋(Wₚ::Matrix; hyper::Dict=Dict())
    ...
    if hyper["lam"] > 0
        ...
        if hyper["lam"] > 1
            ...
            λ₋ ./= ns₊ .+ ns₋
            if hyper["lam"] > 2
                # NaN from div zero which means there are no regulators of a node.
                noreg = ns₊ .+ ns₋ .== 0
