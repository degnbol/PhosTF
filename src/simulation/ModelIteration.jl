...
"""
step(X::Matrix, W, cs::NamedTuple, C::Matrix) = _B(cs,W) * X .* cs.U .+ C
step(X::Matrix, W, cs::NamedTuple, C::Matrix, E::Matrix) = (_B(cs,W) * X + E) .* cs.U .+ C
step!(X::Matrix, W, cs::NamedTuple, C::Matrix) = X .= _B(cs,W) * X .* cs.U .+ C
step!(X::Matrix, W, cs::NamedTuple, C::Matrix, E::Matrix) = X .= (_B(cs,W) * X + E) .* cs.U .+ C


function converge(W, constants::NamedTuple, C=random_C(constants.U), X₀=C; tolerance=default_tolerance, max_iterations=default_max_iterations)
	Xₜ = Xₜ₋₁ = X₀
	difference = nothing
