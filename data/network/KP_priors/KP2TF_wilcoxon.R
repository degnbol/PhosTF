...
for(kp in kps) {
    ...
    DT = DT[, c(wilcox.test(abs(M)[Regulon], abs(M)[!Regulon], alternative="g", conf.int=TRUE)[c("p.value", "estimate")], 
                .(sign=sign(wilcox.test(M[Regulon], M[!Regulon], conf.int=TRUE)$estimate))), by=TF]
