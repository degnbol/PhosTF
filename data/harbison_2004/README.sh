...
sed 1d raw/pvalbygene_forpaper_abbr_YPD.txt | tr -d '\r' | head -n6290 | sed -E '/^[[:space:]]+$/d' | sed 's/_YPD//g' > YPD.tmp
sed 1d raw/pvalbygene_forpaper_abbr_other_conditions.txt | tr -d '\r' | sed -E '/^[[:space:]]+$/d' | cut -f1,4- > conds.tsv
