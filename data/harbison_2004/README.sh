...
sed 1d raw/harbison_2004/pvalbygene_forpaper_abbr_other_conditions.txt | tr -d '\r' | sed -E '/^[[:space:]]+$/d' | cut -f1,4- > conds.tsv
sed 1d YPD.tmp | cut -f1,2 > gene2ORF.tsv
cut -f1,4- < YPD.tmp > YPD.tsv
rm YPD.tmp
...
`git root`/results/4-*/gene2ORF.R YPD.head YPD_ORF.head `git root`/results/4-*/gene2ORF.tsv
r YPD=conds
