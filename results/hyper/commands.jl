...
@threads for (n, i, rep, _cancel, _mean_k, _vec, _norm) in collect(product([10, 100], 1:5, 1:5, [false, true], [false, true], [false, true], 0:2))
    ...
    network(fnames...; header=true, o="$netdir/net$SUF.bson")

    println("sim $n $i $rep $hyper")
    simdir = "sim_logFCs/$hyper" 
    mkpath(simdir)
    logFC("$netdir/net$SUF.bson", "$simdir/sim_logFC$SUF.tsv");
    ...
    for λBstar ∈ [0, .1, .5, 1.]
        for λabsW ∈ [0, .1, .5, 1.]
            infer("$simdir/sim_logFC$SUF.tsv", r"^T[0-9]+$", r"^P[0-9]+$", "$infdir/WT$SUF-$λBstar-$λabsW.adj", "$infdir/WP$SUF-$λBstar-$λabsW.adj"; log="$logdir/log$SUF-$λBstar-$λabsW.tsv", lambda_Bstar=λBstar, lambda_absW=λabsW)
            run(`./rocauc.sh $hyper $λBstar-$λabsW`)
        end
    end
end
