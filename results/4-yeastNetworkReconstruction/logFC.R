...
rootpath = function(path) paste0(ROOT, path)
...
setwd(rootpath("results/4-yeastNetworkReconstuction/"))

fnames = c("data/holstege_2010/PK_KO.tsv",
           "data/holstege_2014/KO.tsv",
           "data/luscombe_2010/TF_KO.tsv",
           "data/luscombe_2010/PK_KO.tsv",
           "data/fiedler_2009/avg_KO.tsv",
           "data/zelezniak_2018/PK_KO.tsv",
           "data/chua_2006/TF_KO.tsv",
           "data/chua_2006/TF_OE.tsv")
pert_tables = list()
for (i in 1:length(fnames)) {
    pert_tables[[i]] = fread(rootpath(fnames[i]), quote="", header=TRUE, sep='\t')
}
OE_fname = grep("OE", fnames)
# make sure to separate OE from KO by appending " OE" to column names from OE file.
colnames(pert_tables[[OE_fname]])[2:ncol(pert_tables[[OE_fname]])] = paste(colnames(pert_tables[[OE_fname]])[2:ncol(pert_tables[[OE_fname]])], "OE")

# melt and concat tables
pert_melt = list()
for (i in 1:length(pert_tables)) {
    pert_melt[[i]] = melt(pert_tables[[i]], id.vars="ORF", variable.name="Mutant")
}
pert_melt = bind_rows(pert_melt)
# should be data.table not data.frame
stopifnot(length(class(pert_melt))==2)
...
fwrite(pert_outer_updated, "logFC_outer.csv", sep=",", quote=F)
fwrite(pert_inner_updated, "logFC_inner.csv", sep=",", quote=F)
# enhancing.
# reducing KOs
reduction = 1
...
fwrite(pert_outer_updated, paste0("logFC_outer_", reduction, ".csv"), sep=",", quote=F)
fwrite(pert_inner_updated, paste0("logFC_inner_", reduction, ".csv"), sep=",", quote=F)
